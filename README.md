## Tpp_Shln_BossEvents
#### Objective
The main goal of this project is to provide MG Sahelanthopus with all information it needs to operate in missions and areas not intended by the original developers.

#### Currently Supported Areas And Stage Versions

| Stage Version  | Support Status  |
| ------------ | ------------ |
| Hellbound AI | Partially Supported  |
| Dominion AI | Not Supported  |
| Dominion AI (Extreme)  | Not Supported  |

You can read more about the stage types [Here](https://mgsvmoddingwiki.github.io/AI/mgs/#stage-types "Here")

Afghanistan Map Regions

| Region  | Support Status  | Description |
| ------------ | ------------ | ------------ |
| Northern Area  | Partially Supported  | Starts at outpost 01 includes OKB 0 entrance, sovietBase until outpost 02  |
| Outpost 04  | Not Supported  |  Starts at outpoost 04, roams the original mission area, 04 area and shifap ruins  |
| Yakho Oboo  | Not Supported  |  Starts at Yakho Oboo C.P., follows to outpost 05 and 12, roams the cp area  |
| Lamar Khaate  | Not Supported  | Starts at Lamar Palace, includes outpost 12, 22 and area in-between  |
| Shago Kallai  | Not Supported  | Starts at Shago Kallai,  roams the Area |
| Wakh Barracks  | Not Supported  | Starts at Wakh Barracks, roams the area from outpost 09 to end of west side of CP |
| Da wiallo Kallai  | Not Supported  | Starts at Da wiallo, roams the area of the CP |
| Da Ghwandai Khar  | Not Supported  | Starts at Da Ghwandai, roams the CP area |
| Qaria Sakhra Ee  | Not Supported  | Starts at Qarya Sakhra, roams the CP area |
| Mountain Relay Base  | Not Supported  | Starts at mountain base, roams the CP area |

Middle Africa Map Regions

**None**

#### Requirements
Infinite Heaven ( [Nexus Mods link](https://www.nexusmods.com/metalgearsolidvtpp/mods/45 "Nexus Mods link"))
IHHook ([Nexus Mods link](https://www.nexusmods.com/metalgearsolidvtpp/mods/1226 "Nexus Mods link"))
Snakebite / Makebite ([Nexus Mods link](https://www.nexusmods.com/metalgearsolidvtpp/mods/106 "Nexus Mods link"))